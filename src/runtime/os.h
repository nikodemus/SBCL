 ...
 * NULL, just tries to map anywhere. If fixedp is true, the address
 * is a strict requirement, if false, it is only considered a hint.
 * Mapped memory is zero-filled.
 *
 * Note: using fixedp=1 after user code has run is inherently dangerous
 ...
 * we use errno=ENOMEM.
 */
extern os_vm_address_t os_validate(os_vm_address_t addr, os_vm_size_t len, boolean fixedp);
